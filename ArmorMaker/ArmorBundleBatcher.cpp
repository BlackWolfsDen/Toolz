    ...
    cin.ignore();
        ...
        SQLfile <<"\n-- "<< ClassName[Pclass] <<" ";
            for(t=1;t<=Tiers;t=t+1)
            {
                ...
                for(i=1;i<=8;i=i+1)
                {
                   SQLfile << ",\n("<< ((EntryIdStrt+((100*t)-100)+i)+((Pclass*10)-(10))) <<", "<< Iclass <<", "<< ClassData[Pclass][1] <<", '"<< ClassName[Pclass] <<"_"<< InvName[i] <<"_T"<< t <<"', "<< DispId[Pclass][t][i] <<", "<< Iquality <<", 1, "<< InvData[i] <<", " << ClassData[Pclass][0] <<", "<< Armorlvl <<", "<< Statcnt <<"";

...
int main()
{
    ...
    cout<<"This will belch out a player exp for level values from 80 up to 255 in an sql file.\n";
    ...
    ofstream SQLfile;
    SQLfile.open ("player_xp_for_level_80-255.sql");
    SQLfile << "REPLACE INTO `player_xp_for_level` (`lvl`, `xp_for_next_level`) VALUES\n";
    SQLfile <<"\n-- ";
            for(l=80;l<=255;l=l+1)
            {
                 SQLfile << ",\n("<< l <<", "<< xpr <<")";
                 xpr=xpr+((xpf-xps)/(255-80));
